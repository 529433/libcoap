...
coap_pdu_duplicate(const coap_pdu_t *old_pdu,
  ...
  pdu = coap_pdu_init(old_pdu->type,
                      old_pdu->code,
...
coap_add_token(coap_pdu_t *pdu, size_t len, const uint8_t *data) {
  ...
  pdu->token_length = (uint8_t)(len + bias);
  if (len) {
    switch (bias) {
    case 0:
...
coap_update_token(coap_pdu_t *pdu, size_t len, const uint8_t *data) {
  ...
  if ((len + bias) == pdu->token_length) {
    ...
    if (!coap_pdu_check_resize(pdu,
                           pdu->used_size + (len + bias) - pdu->token_length)) {
      coap_log(LOG_WARNING, "Failed to update token\n");
      return 0;
    }
    memmove(&pdu->token[(len + bias) - pdu->token_length],
            pdu->token, pdu->used_size);
    pdu->used_size += len + bias - pdu->token_length;
  }
  else {
    pdu->used_size -= pdu->token_length - (len + bias);
    memmove(pdu->token, &pdu->token[pdu->token_length - (len + bias)], pdu->used_size);
  }
  if (pdu->data) {
...
coap_pdu_parse_header(coap_pdu_t *pdu, coap_proto_t proto) {
  ...
  if (e_token_length < COAP_TOKEN_EXT_1B_TKL) {
    pdu->token_length = e_token_length;
    pdu->actual_token.length = pdu->token_length;
    pdu->actual_token.s = &pdu->token[0];
  }
  else if (e_token_length == COAP_TOKEN_EXT_1B_TKL) {
    pdu->token_length = pdu->token[0] + COAP_TOKEN_EXT_1B_BIAS + 1;
    pdu->actual_token.length = pdu->token_length - 1;
    pdu->actual_token.s = &pdu->token[1];
  }
  else if (e_token_length == COAP_TOKEN_EXT_2B_TKL) {
    pdu->token_length = ((uint16_t)pdu->token[0] << 8) + pdu->token[1] +
                        COAP_TOKEN_EXT_2B_BIAS + 2;
    pdu->actual_token.length = pdu->token_length - 2;
    pdu->actual_token.s = &pdu->token[2];
  }
  if (pdu->token_length > pdu->alloc_size || e_token_length == 15) {
...
coap_pdu_parse_opt(coap_pdu_t *pdu) {
  ...
  if (pdu->token_length > pdu->used_size) {
